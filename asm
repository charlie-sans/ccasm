-- Edit template by: Dan200
-- ASM Rework by: Lewisk3 (Redxone)

-- Get file to edit
local tArgs = { ... }
if #tArgs == 0 then
	print( "Usage: "..shell.resolve(shell.getRunningProgram()).." <path>" )
	return
end
-- If the PLEB has an old version of CC
if(settings == nil)then
	settings = {}
	settings.get = function(stuff) return false end
end
...
local CompilerLocation = ""

local tLines = {}
local bRunning = true

-- Colours
local highlightColour, keywordColour, commentColour, textColour, bgColour, stringColour, labelColour, highlightBack
if term.isColour() then
	bgColour = colours.grey
	textColour = colours.white
	highlightColour = colours.lightGrey
	highlightBack = bgColour
	keywordColour = colours.orange
	commentColour = colours.blue
	stringColour = colours.red
	labelColour = colors.lime
	beginColour = colors.purple
	brakColour = colors.lightGray
	accColour   = colors.yellow
else
	...
	beginColour = colors.white
	accColour = colors.white
	brakColour = colors.white
end
...
-- ACC highlighting is near impossible, so screw it.
local function writeHighlighted(sLine)
    while string.len(sLine) > 0 do
        sLine =
            tryWrite(sLine, "^%a*begin%a*", beginColour) or
            tryWrite(sLine, "^%-%-%[%[.-%%]%]", commentColour) or
            tryWrite(sLine, "^#[^\n]*", stringColour) or
            tryWrite(sLine, "^%$[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]", stringColour) or
            tryWrite(sLine, "^%$[0-9a-fA-F]", stringColour) or
            ...
            tryWrite(sLine, "^%(", brakColour) or
            tryWrite(sLine, "^%)", brakColour) or
            tryWrite(sLine, "^[%w_]+", function(match)
                if tKeywords[string.lower(match)] then
...
local function complete( sLine )
	if settings.get and settings.get( "edit.autocomplete" ) then
...
local function redrawMenu()
	...
	if bMenu then
        ...
        -- Draw status
        term.setTextColour( highlightColour )
        term.setBackgroundColor( highlightBack )
        term.write( sStatus )
         term.setBackgroundColor( bgColour )
        term.setTextColour( textColour )
    end
...
--print("Thanks for using ASM-Edit! (6502)")
